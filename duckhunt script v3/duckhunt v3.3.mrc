; ============================================================
; DuckHunt Auto-Shooter Script v3.3
...
on *:TEXT:*QUACK!*:#:{
  ...
  set %duck.chan $chan
  .timer duck_ $+ $chan 1 %delay msg $chan !bang
...
on *:TEXT:*out of ammo*:#:{
  ...
  .timer duck_reload_ $+ $chan 1 2 msg $chan !reload
  .timer duck_ $+ $chan 1 4 msg $chan !bang
}

; ============================================================
; Duck escaped — cancel timers and clean up
; ============================================================
on *:TEXT:*flies away*:#:{
  if (!$duck_chans($chan))  { halt }
  if (!$duck_isbot($nick))  { halt }
  set %duck.active. [ $+ $chan ] 0
  .timer duck_ $+ $chan off
...
on *:TEXT:*DECOY DUCK*:#:{
  ...
  .timer duck_decoy_ $+ $chan 1 %delay msg $chan !befriend
