; mIRC DuckHunt Auto-Reply Script v3.1
; ============================================================
; CHANGELOG:
...
; v3.1  Three bugs found from live log analysis (#3nd3r session, 2026-02-27):
;
;   BUG 1 - Watchdog echo printed "$lower($chan)" literally instead of evaluating it.
;       The watchdog block fires before %c is assigned, so $lower($chan) inside
;       $+ concatenation was not being evaluated by mIRC - it echoed as a raw string.
;       Also the channel name was missing from the addchan suggestion at the end.
;       Fix: assign %wc = $lower($chan) locally before the echo and use %wc throughout.
;
;   BUG 2 - Slug function broke channels whose name begins with a digit (e.g. #3nd3r).
;       duckhunt.slug(#3nd3r) returned "3nd3r". mIRC variable names cannot begin with
;       a digit after an interpolated $+ join - the token got split/corrupted, producing
;       garbage like "2 3 3nd3r" in the channel list instead of "#3nd3r". This caused
;       $istok() to never match the channel, so duckhunt.active() always returned 0
;       and every duck in #3nd3r hit the watchdog branch instead of the hunt handler.
;       Fix: duckhunt.slug() now prefixes digit-leading names with "ch" so #3nd3r
;       becomes slug "ch3nd3r", giving a valid variable name %duckhunt.ch3nd3r.duck.active.
;       Timer names use the same slug so they are also safe.
;
;   BUG 3 - addchan stored malformed token in %duckhunt.channels.
;       Related to Bug 2: because the slug was digit-leading, the $addtok call that
;       builds %duckhunt.channels was receiving a corrupted value for #3nd3r channels.
;       Fix: %duckhunt.channels always stores the full lowercased #channel name (the
;       slug is only used for variable/timer name suffixes, never as the list token).
;       This was already the intent of v3.0 but the slug corruption in Bug 2 was
;       propagating into the list. Confirmed clean with the slug prefix fix.
; ============================================================

; ============================================================
; INTERNAL: channel slug  (#url -> url, #3nd3r -> ch3nd3r)
; Used to build variable and timer names ONLY - never stored
; in %duckhunt.channels (which always holds the full #name).
; mIRC variable names cannot begin with a digit after a $+
; join, so channels like #3nd3r get a "ch" prefix to keep
; the resulting name (e.g. %duckhunt.ch3nd3r.duck.active) valid.
; ============================================================
alias duckhunt.slug {
  var %raw = $lower($mid($1, 2))
  if ($left(%raw, 1) isnum) { return ch $+ %raw }
  return %raw
}
...
on *:TEXT:*:*:{
  ...
  if (!$duckhunt.active($chan)) {
    ...
    ; %wc is assigned here explicitly - do NOT use $lower($chan) inside $+ concatenation
    ; as mIRC will not evaluate the function call and will echo it as a literal string.
    ...
    if (*QUACK* iswm $1-) || (*quack* iswm $1-) || (*\_O<* iswm $1-) || (*\_o<* iswm $1-) || (*flap flap* iswm $1-) || (*flock of*ducks* iswm $1-) || (*BOSS DUCK* iswm $1-) {
      var %wc = $lower($chan)
      echo -a *** [ $+ %wc $+ ] WARNING: DUCK SPAWNED BUT CHANNEL IS NOT TRACKED - run /duckhunt addchan %wc
    }
    return
  }
...
; LOAD MESSAGE
; ============================================================
echo -a *** DuckHunt Auto-Reply Script Loaded (v3.1 - Multichannel)
