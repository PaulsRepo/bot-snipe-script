; mIRC DuckHunt Auto-Reply Script v2.4
...
; - v2.4: Fix buy-back not triggering on confiscation - "missed and hit" and
;         [GUN CONFISCATED] arrive in the SAME message; old code waited for a
;         follow-up message that never came, so buy-back was skipped entirely
...
on *:TEXT:*:#url:{
  ...
  if ($me isin $1-) {
    ...
    ; Missed and hit someone - GUN CONFISCATED is in the same message, handle it immediately
    elseif (*missed and hit* iswm $1-) {
      ...
      ; Check if we have Buy Gun Back in inventory - act immediately, don't wait for a follow-up message
...
echo -a *** DuckHunt Auto-Reply Script Loaded (v2.4)
